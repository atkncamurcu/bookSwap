...
import {Image, ScrollView, Text, TouchableOpacity, View, TextInput,Platform,KeyboardAvoidingView} from 'react-native';
...
export default class Chat extends React.Component {
    ...
    async sendMessage(){
        const { navigation } = this.props;
        const trade_id = navigation.getParam("trade_id", "NO-ID");
        await client.post("/user/chat/messages/send",{trade_id : trade_id, body: this.state.user_message});
    ...
    async getMessagesList(){
        ...
        let {data : messagesList} = await client.get("/user/chat/messages/" + trade_id + "/" + trader_id);
            ...
            onContentSizeChange={() => this.scrollView.scrollToEnd({animated: true})}>              
              {this.state.messagesList.map((item, index) => (
                this.state.user_id == item.owner_user_id ? (
                  <View style={{justifyContent:"center",alignSelf: 'flex-start', backgroundColor: 'rgba(109,118,247, 0.2)',margin:10,borderRadius:15}} key={index}>
                    <Text style={{ borderTopRightRadius:7,borderBottomRightRadius:7,padding:10,fontSize:15,color:'white',textAlign:'center',marginEnd:5}}>
                         {item.body}
                      </Text>  
                  </View>
                ):(
                  <View style={{justifyContent:"center",alignSelf: 'flex-end', backgroundColor: 'rgba(109,118,247, 0.5)',margin:10,borderRadius:15}} key={index}>
                      <Text style={{borderTopLeftRadius:7,borderBottomLeftRadius:7,padding:10,fontSize:15,color:'white',textAlign:'center',marginStart:5}}>
                         {item.body}
                      </Text>
                  </View>
                )
